...
export async function POST(request) {
  ...
  try {
    ...
    const event = await NTEvent.findOne({ eventid: eventId });
    if (!event) {
    ...
    const generateUniqueTransactionId = async () => {
      ...
      while (!isUnique) {
        generatedId = `TXN_YUK_NTE_${Date.now()}_${Math.floor(
    ...
    const paymentParams = {
      ...
      fees: event.fees,
      returnurl: `${process.env.NEXT_PUBLIC_APP_URL}/api/payment/ntevent/callback`,
      provider: 2,
    };
